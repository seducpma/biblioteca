class GenerosController < ApplicationController
  ...
  # GET /generos.xml
  def index
    @generos = Genero.paginate :page => params[:page], :order => 'nome ASC', :per_page => 20
...
def consultaGen
  unless params[:search].present?
    if params[:type_of].to_i == 3
      ...
      @generos = Genero.paginate :all,:page => params[:page], :order => 'nome ASC', :per_page => 20
    ...
    if params[:type_of].to_i == 1
      ...
      @generos = Genero.paginate :all, :page => params[:page], :per_page => 20, :conditions => ["nome like ?", "%" + params[:search].to_s + "%"],:order => 'nome ASC'
      render :update do |page|
        page.replace_html 'generos', :partial => "generos"
      end
    end
